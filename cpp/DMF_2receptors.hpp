...
void checkParams(const ParamStruct &params) {
    ...
    std::vector<std::string> required_fields = {"C", "receptors_1", "receptors_2", "dt",
            "taon", "taog", "gamma", "sigma", "JN", "I0", "Jexte", "Jexti",
            "w", "g_e", "Ie", "de", "g_i", "Ii", "di", "scale_1", "scale_2", "bias",
            "G", "TR", "dtt", "batch_size"};
...
public:
    ...
    Eigen::ArrayXd sn, sg, J, receptors_1, receptors_2, Jexte, Jexti, scale_1, scale_2, bias;
    ...
    DMFSimulator(ParamStruct params, size_t nb_steps_in, size_t N_in,
            ...
            bold_int(params, nb_steps, N_in) {
              ...
              receptors_1 = ensureArray(params, "receptors_1", N);
              receptors_2 = ensureArray(params, "receptors_2", N);
              scale_1     = ensureArray(params, "scale_1", N);              
              scale_2     = ensureArray(params, "scale_2", N);
    ...
    inline Eigen::ArrayXd curr2rate(const Eigen::ArrayXd& x, const Eigen::ArrayXd& scale_1, const Eigen::ArrayXd& scale_2, const Eigen::ArrayXd& bias, double g,
           double I, double d) {
        Eigen::ArrayXd y = g*(x-I)*(1+receptors_1*scale_1+receptors_2*scale_2+bias);
        return y/(1-exp(-d*y));
    }
    ...
    void run(double* rate_res, double* bold_res) {
        ...
        for (size_t t = 0; t < nb_steps; t++) {
            ...
            for (size_t dummy = 0; dummy < steps_per_millisec; dummy++) {
                ...
                rn.col(rate_idx) = curr2rate(xn, scale_1,scale_2,bias, g_e, Ie, de);
                Eigen::ArrayXd rg = curr2rate(xg, scale_1,scale_2,bias, g_i, Ii, di);
