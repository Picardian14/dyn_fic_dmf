#!/bin/bash
#SBATCH --job-name=cnt_fit_bayesian_static_fcd
...
singularity exec $SINGULARITY_IMAGE bash -c 'source /opt/anaconda/3/2023.07-2/base/etc/profile.d/conda.sh && conda activate fic && cd dyn_fic_dmf/python && python3 cnt_fit_bayesian_static_fcd.py ' > ./g_lr.out 