...
public:
  ...
  DMF(bp::dict d, size_t nb_steps, size_t N, bool return_rate, bool return_bold, bool return_fic) :
    DMFSimulator(struct2map(d), nb_steps, N, return_rate, return_bold, return_fic) {}

  void run(np::ndarray rate_e_res,np::ndarray rate_i_res, np::ndarray bold_res,np::ndarray fic_res) {
    DMFSimulator::run(safeGet(rate_e_res),safeGet(rate_i_res),safeGet(bold_res), safeGet(fic_res));
  }
