function [results] = exploreBehavior(G, lr,G_span,dmf_pars,opts)
    ...
    if length(G(:))==2 
        Gvals = optimizableVariable('G',[G(1) G(2)], 'Transform','log');
        opt_vars = [Gvals];
    end
    ...
    results = bayesopt(@aux_dyn_fic_dmf,opt_vars,opts{:});
        ...
        function [homeostatic_fittness, const, outdata] = aux_dyn_fic_dmf(G_lr_params)
            const = [];
            dmf_pars.taoj = G_lr_params.G;
            dmf_pars.lrj = G_lr_params.lr;
