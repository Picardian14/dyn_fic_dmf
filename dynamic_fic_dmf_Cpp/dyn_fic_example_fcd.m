...
params.J = params.alpha*params.G*sum(params.C, 1)' + 1; % FIC initial value
...
% Computing FCd
T = length(bold);
isubfcd = find(tril(ones(params.N),-1));
flp = 0.01;
fhp = 0.1;
wsize = 30;
overlap = 29;
...
mean_fic_dist = zeros(params.N,1);
for n=1:N
    data = fic_t(:,n);
%     data(data<=0) = eps;
